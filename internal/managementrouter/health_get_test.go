...
var _ = Describe("GetHealth", func() {
	...
	Context("when GetAlertingHealth returns an error", func() {
		...
		It("should return 500 via handleError", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(errResp["error"]).To(Equal("An unexpected error occurred"))
		})
	})
})
...
func (s *healthStubManagementClient) ListRules(ctx context.Context, prOptions management.PrometheusRuleOptions, arOptions management.AlertRuleOptions, pgOptions management.PaginationOptions) (management.ListRulesResult, error) {
