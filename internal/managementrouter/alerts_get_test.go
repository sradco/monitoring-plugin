...
func TestGetAlerts_ParsesFlatQueryParams(t *testing.T) {
	...
	assertLabelValues(t, captured.Labels, "namespace", []string{"ns1"})
	assertLabelValues(t, captured.Labels, "severity", []string{"critical"})
	assertLabelValues(t, captured.Labels, "team", []string{"sre"})
}

func TestGetAlerts_MultiValueLabelFilter(t *testing.T) {
	f := newAGFixture(t)
	var captured k8s.GetAlertsRequest
	f.mockPrometheusAlerts.FetchAlertsFunc = func(_ context.Context, req k8s.GetAlertsRequest) ([]k8s.PrometheusAlert, []string, error) {
		captured = req
	...
	w := f.get(t, "/api/v1/alerting/alerts?severity=critical&severity=warning")

	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", w.Code, w.Body)
	}
	assertLabelValues(t, captured.Labels, "severity", []string{"critical", "warning"})
}

func assertLabelValues(t *testing.T, labels map[string][]string, key string, want []string) {
	t.Helper()
	got := labels[key]
	if len(got) != len(want) {
		t.Errorf("label %q: expected %v, got %v", key, want, got)
		return
	}
	for i := range want {
		if got[i] != want[i] {
			t.Errorf("label %q[%d]: expected %q, got %q", key, i, want[i], got[i])
		}
	}
}
