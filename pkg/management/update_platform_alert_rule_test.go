...
var _ = Describe("UpdatePlatformAlertRule", func() {
	...
	Context("when trying to update a non-platform rule", func() {
		...
		It("returns an error when user workload ARCs are disabled", func() {
			updatedRule := userRule
			err := client.UpdatePlatformAlertRule(ctx, userRuleId, updatedRule)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("ENABLE_USER_WORKLOAD_ARCS"))
		})
	})
...
var _ = Describe("Drop/Restore Platform Alert Rule", func() {
	...
	It("deletes ARC when restore leaves only the id-stamp (stamp+Drop scenario)", func() {
		deleted := false
		var updated *osmv1.AlertRelabelConfig
		...
		err := client.RestorePlatformAlertRule(ctx, drPlatformRuleId)
		Expect(err).NotTo(HaveOccurred())
		Expect(deleted).To(BeTrue(), "ARC should be deleted when only stamp remains after removing Drop")
		Expect(updated).To(BeNil(), "ARC should not be updated, it should be deleted")
	})
})
