...
func alertingRuleEnabledChange(enabled bool) []RuleChange {
	if enabled {
		return []RuleChange{{
...
func findAlertingRuleIndices(ar *osmv1.AlertingRule, alertName string) (groupIdx, ruleIdx int, found bool) {
	for gi := range ar.Spec.Groups {
		for ri := range ar.Spec.Groups[gi].Rules {
			if ar.Spec.Groups[gi].Rules[ri].Alert == alertName {
				return gi, ri, true
			}
		}
	}
...
func buildDropRestoreRuleChangePlan(
	...
	changes := alertingRuleEnabledChange(enabled)
	if desiredObject == nil && !enabled {
		changes = append(changes, RuleChange{
			Field:     "resource",
			Operation: RuleChangeOpRemove,
		})
