...
func (c *client) migrateClassificationOverrideIfRuleIDChanged(
	...
	oldArcName := k8s.GetAlertRelabelConfigName(prometheusRuleName, oldRuleId)
	arcNamespace := k8s.UserWorkloadMonitoringNamespace

	oldArc, found, err := c.k8sClient.AlertRelabelConfigs().Get(ctx, arcNamespace, oldArcName)
	if err != nil {
		return err
	}
	if !found {
	...
	existingOverrides, _ := collectExistingFromARC(true, oldArc)
	existingRuleDrops := getExistingRuleDrops(oldArc, oldRuleId)

	if err := c.k8sClient.AlertRelabelConfigs().Delete(ctx, arcNamespace, oldArcName); err != nil {
		return fmt.Errorf("failed to delete old AlertRelabelConfig %s/%s: %w", arcNamespace, oldArcName, err)
	}

	if len(existingOverrides) == 0 && len(existingRuleDrops) == 0 {
		return nil
	}

	pr, prFound, err := c.k8sClient.PrometheusRules().Get(ctx, ruleNamespace, prometheusRuleName)
	if err != nil {
		return err
	}
	if !prFound {
	...
	var updatedRule *monitoringv1.Rule
	for groupIdx := range pr.Spec.Groups {
		for ruleIdx := range pr.Spec.Groups[groupIdx].Rules {
			r := &pr.Spec.Groups[groupIdx].Rules[ruleIdx]
			if ruleMatchesAlertRuleID(*r, newRuleId) {
				updatedRule = r
				break
			}
		}
	...
	original := copyStringMap(updatedRule.Labels)
	desired := buildDesiredLabels(original, existingOverrides)
	nextChanges := buildNextLabelChanges(original, desired)

	newArcName := k8s.GetAlertRelabelConfigName(prometheusRuleName, newRuleId)
	relabelConfigs := buildRelabelConfigs(updatedRule.Alert, original, newRuleId, nextChanges)
	relabelConfigs = appendPreservedRuleDrops(relabelConfigs, existingRuleDrops)

	return upsertAlertRelabelConfig(c.k8sClient, ctx, arcNamespace, newArcName, prometheusRuleName, updatedRule.Alert, newRuleId, false, nil, relabelConfigs)
}
