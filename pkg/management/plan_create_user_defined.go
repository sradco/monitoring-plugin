...
func (p *createUserDefinedPlan) toRuleChangePlan() *RuleChangePlan {
	desiredPR := buildDesiredPrometheusRuleWithAddedRule(p.existingPR, p.groupName, p.groupIdx, p.preparedRule)
	if desiredPR.ObjectMeta.Namespace == "" {
		desiredPR.Namespace = p.nn.Namespace
	}
	if desiredPR.ObjectMeta.Name == "" {
		desiredPR.Name = p.nn.Name
	}
	desiredObject, _ := prometheusRuleDesiredObject(desiredPR)
