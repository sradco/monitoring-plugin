...
type GetAlertsRequest struct {
	// Labels filters alerts by labels. Each key maps to one or more accepted
	// values (OR semantics): an alert matches when its label value equals any
...
func (pa *prometheusAlerts) FetchAlerts(ctx context.Context, req GetAlertsRequest) ([]PrometheusAlert, []string, error) {
	...
	if err != nil {
		...
		if namespaceFromMultiLabels(req.Labels) == "" {
			return nil, nil, err
		}
...
func (pa *prometheusAlerts) getUserWorkloadAlerts(ctx context.Context, req GetAlertsRequest) ([]PrometheusAlert, error) {
	...
	namespace := namespaceFromMultiLabels(req.Labels)
	if namespace != "" {
...
// namespaceFromMultiLabels extracts the namespace from multi-valued label
// filters, returning the first non-empty value.
func namespaceFromMultiLabels(labels map[string][]string) string {
	if labels == nil {
		return ""
	}
	vals := labels["namespace"]
	if len(vals) == 0 {
		return ""
	}
