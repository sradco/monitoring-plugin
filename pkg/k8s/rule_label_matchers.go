...
// labelsMatch returns true when every label key in the request is present on
// the alert and the alert's value is one of the accepted values for that key
// (OR semantics within a key, AND across keys).
func labelsMatch(req *GetAlertsRequest, alert *PrometheusAlert) bool {
	for key, values := range req.Labels {
		...
		if !slices.Contains(values, alertValue) {
			return false
		}
	}
	return true
