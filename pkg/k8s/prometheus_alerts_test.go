...
func TestLabelsMatch(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			req:   map[string][]string{"severity": {"critical", "warning"}},
			alert: map[string]string{"severity": "warning"},
			want:  true,
		},
		{
			name:  "multi-value OR no match",
			req:   map[string][]string{"severity": {"critical", "warning"}},
			alert: map[string]string{"severity": "info"},
			want:  false,
		},
		{
			name:  "multi-value on one key AND single on another",
			req:   map[string][]string{"severity": {"critical", "warning"}, "namespace": {"ns-a"}},
			alert: map[string]string{"severity": "critical", "namespace": "ns-a"},
			want:  true,
		},
		{
			name:  "multi-value match but other key fails",
			req:   map[string][]string{"severity": {"critical", "warning"}, "namespace": {"ns-b"}},
			alert: map[string]string{"severity": "critical", "namespace": "ns-a"},
			want:  false,
		},
	}
...
// --- namespaceFromMultiLabels ---

func TestNamespaceFromMultiLabels(t *testing.T) {
	cases := []struct {
		name   string
		labels map[string][]string
		want   string
		...
		{"nil labels", nil, ""},
		{"missing namespace", map[string][]string{"other": {"val"}}, ""},
		{"empty values", map[string][]string{"namespace": {}}, ""},
		{"present namespace", map[string][]string{"namespace": {"test-ns"}}, "test-ns"},
		{"whitespace-only namespace", map[string][]string{"namespace": {"  "}}, ""},
		{"padded namespace", map[string][]string{"namespace": {" ns-a "}}, "ns-a"},
		{"multiple namespaces uses first", map[string][]string{"namespace": {"ns-a", "ns-b"}}, "ns-a"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := namespaceFromMultiLabels(tc.labels)
			if got != tc.want {
				t.Errorf("namespaceFromMultiLabels(%v) = %q, want %q", tc.labels, got, tc.want)
			}
		})
	}
}
