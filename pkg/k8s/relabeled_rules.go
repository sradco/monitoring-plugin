...
func newRelabeledRulesManager(ctx context.Context, namespaceManager NamespaceInterface, alertRelabelConfigs AlertRelabelConfigInterface, monitoringv1clientset *monitoringv1client.Clientset, clientset *kubernetes.Clientset) (*relabeledRulesManager, error) {
	...
	if !cache.WaitForNamedCacheSync("RelabeledRulesConfig informer", ctx.Done(),
	...
	for _, obj := range rrm.prometheusRulesInformer.GetStore().List() {
		...
		for _, group := range promRule.Spec.Groups {
			for _, rule := range group.Rules {
				...
				clonedLabels := make(map[string]string, len(rule.Labels)+5)
				for k, v := range rule.Labels {
					clonedLabels[k] = v
				}
				rule.Labels = clonedLabels
				...
				if rrm.namespaceManager.IsClusterMonitoringNamespace(promRule.Namespace) {
					lb := labels.NewBuilder(labels.FromMap(rule.Labels))
...
func sanitizeDNSName(in string) string {
	...
	res := strings.Trim(multiDash.ReplaceAllString(string(out), "-"), "-")
	if res == "" {
		return "arc"
	}
	return res
...
func (rrm *relabeledRulesManager) determineManagedBy(ctx context.Context, promRule *monitoringv1.PrometheusRule, alertRuleId string) (string, string) {
	...
	if alertingRuleOwner(promRule) == "" {
		// Prefer operator-managed over GitOps when owner references indicate an operator
		gitOpsManaged, operatorManaged := IsExternallyManagedObject(promRule)
		if operatorManaged {
...
// DetermineManagedBy determines the managed-by labels for a PrometheusRule alert rule.
func DetermineManagedBy(ctx context.Context, alertRelabelConfigs AlertRelabelConfigInterface, namespaceManager NamespaceInterface, promRule *monitoringv1.PrometheusRule, alertRuleId string) (string, string) {
	rrm := &relabeledRulesManager{
		alertRelabelConfigs: alertRelabelConfigs,
		namespaceManager:    namespaceManager,
	}
...
func (rrm *relabeledRulesManager) List(ctx context.Context) []monitoringv1.Rule {
	...
	result := make([]monitoringv1.Rule, 0, len(rrm.relabeledRules))
	for _, rule := range rrm.relabeledRules {
		result = append(result, deepCopyRule(rule))
	}
...
func (rrm *relabeledRulesManager) Get(ctx context.Context, id string) (monitoringv1.Rule, bool) {
	...
	return deepCopyRule(rule), true
}

func deepCopyRule(r monitoringv1.Rule) monitoringv1.Rule {
	cp := r
	if r.Labels != nil {
