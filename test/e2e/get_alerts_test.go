...
// TestRBAC_GetAlerts verifies Thanos-tenancy RBAC for GET /alerts?namespace=.
//
// User A (no perms): HTTP 200 without the UWM alert in ns Y.
// User B (monitoring-rules-view in Y): sees the alert for Y, not for Z.
// User C (cluster-admin): sees the alert for Y.
