...
type listRulesRuleGroup struct {
...
type listRulesResponse struct {
	Data struct {
		Groups []listRulesRuleGroup `json:"groups"`
	} `json:"data"`
}

func listRules(ctx context.Context, pluginURL string, httpClient *http.Client) ([]monitoringv1.Rule, error) {
...
func TestPrometheusRuleAppearsInMemory(t *testing.T) {
	...
	err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 2*time.Minute, true, func(ctx context.Context) (bool, error) {
		rules, err := listRules(ctx, f.PluginURL, httpClient)
		if err != nil {
