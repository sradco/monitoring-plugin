...
func listRulesForAlertMgmt(ctx context.Context, f *framework.Framework) ([]monitoringv1.Rule, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, f.PluginURL+"/api/v1/alerting/rules", nil)
	if err != nil {
		return nil, err
	}
...
func TestBulkDeleteUserDefinedAlertRules(t *testing.T) {
	...
	err = wait.PollUntilContextTimeout(ctx, 2*time.Second, 2*time.Minute, true, func(ctx context.Context) (bool, error) {
		rules, err := listRulesForAlertMgmt(ctx, f)
		if err != nil {
